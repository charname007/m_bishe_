...
LABEL_USER = """## 任务描述
请为已识别的实体和关系打上专业属性标签（v3.3：特征标签改为开放文本）。

---

## 实体属性标注（v3.3：5个属性，特征标签开放）
...
| 属性 | 类型/示例 | 提取条件 |
|------|--------|----------|
| 推荐指数 | 超推/推荐/一般/不推荐 | 明确推荐/评价语句出现 |
| 情感倾向 | 正面/中性/负面 | 情感词/评价词出现 |
| 特征标签 | **开放文本**：氛围超好、随手拍好看、遛娃神器、松弛感、治愈感等 | 特征描述词出现（保留原文表达） |

**约束**：所有属性必须有原文依据，禁止凭空创造。**特征标签采用开放文本设计，不做标准化归一化**。"""
