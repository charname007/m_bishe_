...
| **相对方位** | A和B空间邻近+相对方位关系 | 地理实体 | **距离值**+**方向值**+**联动推荐**（可选） |
...
{{\"triples\": [
    ...
    {{\"head\": \"群光广场\", \"relation\": \"相对方位\", \"tail\": \"银泰城\", \"evidence\": \"一起在商圈里\", \"attributes\": {{\"联动推荐\": true}}}}
  ]
}}
...
输出:
 {{
  ...
  "triples": [
    ...
    {{\"head": "群光广场", "relation": "相对方位", "tail": "银泰城", "evidence": "一起在商圈里", "confidence": "high", "attributes": {{\"联动推荐\": true}}}}
  ],
...
- **相对方位**：A和B空间邻近+相对方位关系（属性：距离值/方向值/联动推荐）
...
# 关系类型定义（v3.2精简版：8种）
RELATION_SCHEMA_CORE = """## 关系类型定义（v3.2精简版：8种）
...
| 包含 | A空间包含B（位于的反向） | POI/建筑物/道路 | 无 |
| 相对方位 | A和B空间邻近+方位关系 | 地理实体 | 距离值/方向值/联动推荐 |

### 语义血肉关系
| 关系 | 语义定义 | Tail类型 | 核心属性 |
|------|----------|----------|----------|
| 具有功能 | 场所可进行的用途 | 功能节点(9类) | 时段/适合人群/具有限制 |
...
| 关系 | 语义定义 | Tail类型 | 核心属性 |
|------|----------|----------|----------|
| 发生事件 | 场所发生的特定事件 | 事件节点 | (属性在事件节点上) |

**注**：原"相邻"、"距离"、"方向"已合并为"相对方位"。"""
...
| 联动推荐 | true/false | 是否推荐联动游览 |

### 具有功能关系属性
| 属性 | 枚举值 | 说明 |
|------|--------|------|
| 时段 | 周末/晚上/樱花季/春季/夏季等 | 功能适用时段 |
| 适合人群 | 亲子/宝妈/学生党/情侣/打工人/特种兵/银发族/宠物主/独行者/团建 | 功能适合人群 |
| 具有限制 | 需预约/排队久/停车难/限流/谢绝宠物/只收现金/时间限制/人数限制/消费门槛/季节限制 | 功能限制条件 |
| 情感倾向 | 正面/中性/负面 | 功能体验情感 |
| 功能描述 | 自由文本 | 当功能类型=其他时，具体描述功能内容 |

**重要说明：功能"其他"的使用规则**
- 当功能用途无法归入8个核心类型（餐饮/购物/休闲/社交/观景/住宿/文化/工作）时，使用"其他"
- 使用"其他"时，**必须**在"功能描述"字段中说明具体功能内容
...
**重要说明：维度"其他"的使用规则**
- 当对比内容无法归入7个核心维度（价格/环境/服务/人流量/品质/交通/口味）时，使用"其他"
- 使用"其他"时，**必须**在"维度描述"字段中说明具体对比内容
- 示例：原文"这家店装修风格比隔壁好" → 维度=["其他"], 维度描述="装修风格"
- 示例：原文"A比B更有特色" → 维度=["其他"], 维度描述="特色程度"
...
ENTITY_SCHEMA_TABLE = """## 实体类型（GIS标准）
...
RELATION_SCHEMA_TABLE = """## 关系类型（v3.2精简版：8个）

| 类别 | 关系 | Tail类型 | 核心属性 |
|------|------|----------|----------|
| 空间骨架 | 位于 | 道路/街区/行政区 | 无 |
| 空间骨架 | 包含 | POI/建筑物/道路 | 无 |
| 空间骨架 | 相对方位 | 地理实体 | 距离值/方向值/联动推荐 |
| 语义血肉 | 具有功能 | 功能节点(9类) | 时段/适合人群/限制 |
| 对比评价 | 优于 | 地理实体 | 维度列表 |
| 对比评价 | 相似 | 地理实体 | 维度列表 |
| 对比评价 | 劣于 | 地理实体 | 维度列表 |
| 事件 | 发生事件 | 事件节点 | (属性在事件节点上)"""

FUNCTION_SCHEMA_TABLE = """## 功能节点（9大类）
...
### 功能属性
- 时段: 周末/晚上/樱花季等（文本）
- 适合人群: 亲子/学生党/情侣/打工人等（枚举）
- 具有限制: 需预约/排队久/停车难等（多选）
- 情感倾向: 正面/中性/负面
