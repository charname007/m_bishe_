...
| 关系 | 语义定义 | Tail类型 | 关系属性 |
|------|----------|----------|----------|
| 推荐指数 | 用户推荐程度 | 超推/推荐/一般/不推荐 | 无 |
| 承载活动 | 场所可进行的活动 | 活动节点 | **时段**+**适合人群**+**具有限制** |
| 可达方式 | 到达的交通方式 | 地铁/公交/步行/自驾/骑行/打车 | 无 |
| 消费档次 | 消费水平 | 平价/中档/高档/奢侈 | 无 |
| 品类特征 | 风格/文化特征 | 老字号/网红/新潮/文创/ins风/复古 | 无 |
| 引发情感 | 情感体验大类 | 正面/中性/负面 | **具体表达**（可选） |
...
| 关系 | 语义定义 | Tail类型 | 关系属性 |
|------|----------|----------|----------|
| 发生事件 | 场所发生的特定事件 | 事件节点 | **事件类别**+**状态**+**时间** |
...
| 属性 | 枚举值 |
|------|--------|
| 维度 | 价格/环境/服务/人流量/品质/氛围/交通/停车/口味/性价比 |

### 发生事件关系属性
| 属性 | 枚举值 |
|------|--------|
| 事件类别 | 自然事件/人文事件/商业事件/社会事件/负面事件 |
| 状态 | 正在进行/已结束/计划中/周期性 |
