...
# ===== Step 2: RE 提示词模板（v3.2精简版：6个关系体系） =====
...
| **特征标签** | 氛围/定位/体验特征 | 氛围感/网红/热门/服务好/性价比高等（约16个） |
| **交通方式** | 可达交通方式 | 地铁/公交/步行/自驾/骑行/打车 |
| **交通便利度** | 交通便利性评价 | 便利/一般/不便 |
| **消费档次** | 消费水平 | 平价/中档/高档/奢侈 |
...
| 属性 | 枚举值 | 说明 |
|------|--------|------|
| 交通方式 | 地铁/公交/步行/自驾/骑行/打车 | 可达交通方式（列表） |
| 交通便利度 | 便利/一般/不便 | 交通便利性评价 |
| 消费档次 | 平价/中档/高档/奢侈 | 消费水平 |
| 人均消费 | 约50元/人均100等 | 人均消费参考（可选） |
| 特征标签 | 老字号/网红/新潮/文创/ins风/复古/小众/连锁 | 风格特征（列表） |
...
| 包含 | 包含类型[完全包含/部分包含]，边界关系[明确边界/模糊边界] |
| 方位 | 空间精度[精确/近似]，方位类型[距离为主/方向为主/邻近为主] |

### 社交语义关系属性

| 关系 | Label补充属性 |
|------|--------------|
| 具有功能 | 功能类型[高频/中频/低频]，功能频率[日常/周末/季节性/特定时段] |
...
## 原始文本（用于提取情感标签、体验评价、交通方式、消费档次、特征标签）
{raw_text}
...
以下语义应作为**实体属性**而非三元组关系：
| 属性 | 枚举值 |
|------|--------|
| 推荐指数 | 超推/推荐/一般/不推荐 |
| 情感倾向 | 正面/中性/负面 |
| 特征标签 | 氛围感/网红/热门/服务好/性价比高等（约16个） |
| 交通方式 | 地铁/公交/步行/自驾/骑行/打车 |
| 交通便利度 | 便利/一般/不便 |
| 消费档次 | 平价/中档/高档/奢侈 |
...
输出:
 {{
  "entities": [
    {{\"name\": "群光广场", "type": "建筑物", "category": "商业综合体", "aliases": [], "evidence": "群光广场", "attributes": {{\"交通方式\": ["地铁"], \"消费档次\": "中档", \"特征标签\": ["网红"]}}}},
...
输出:
 {{
  ...
  "triples": [
    {{\"head": "书店", "relation": "发生事件", "tail": "业态变更", "evidence": "改成咖啡厅了", "confidence": "high", "attributes": {{\"事件类别": "业态变更", "状态": "已结束"}}}}
  ],
  "entity_relation_mapping": {{
    "书店": ["<书店, 发生事件, 业态变更>"]
  }},
